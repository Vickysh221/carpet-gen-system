...
function averagePreferenceState(states: SimulatorState[]): FirstOrderSlotValues | null {
  ...
  const total = states.reduce(
    ...
    {
      color: { warmth: 0, saturation: 0, lightness: 0 },
      motif: { geometry: 0, organic: 0, complexity: 0 },
      arrangement: { order: 0, spacing: 0, direction: 0 },
    }
  );
  ...
  return {
    color: {
      warmth: total.color.warmth / count,
      saturation: total.color.saturation / count,
      lightness: total.color.lightness / count,
    },
    motif: {
      geometry: total.motif.geometry / count,
      organic: total.motif.organic / count,
      complexity: total.motif.complexity / count,
    },
    arrangement: {
      order: total.arrangement.order / count,
      spacing: total.arrangement.spacing / count,
      direction: total.arrangement.direction / count,
    },
  };
}
  ...
  return (
    <div className={`rounded-2xl border p-3 ${highlighted ? "border-stone-900 bg-white shadow-sm" : "border-stone-200 bg-stone-50"}`}>
      <div className="mb-2 flex items-center justify-between gap-2">
        ...
        <div className={`rounded-full px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] ${tone === "direct" ? "bg-stone-200 text-stone-700" : "bg-amber-100 text-amber-800"}`}>{toneLabel}</div>
      </div>
      <div className="space-y-2">
        {Object.entries(values).map(([key, value]) => (
          <div key={key}>
            <div className="mb-1 flex items-center justify-between text-xs text-stone-600"><span>{key}</span><span>{formatPercent(value)}</span></div>
            <div className="h-2 rounded-full bg-stone-200"><div className={`h-2 rounded-full ${barClass}`} style={{ width: `${value * 100}%` }} /></div>
          </div>
        ))}
      </div>
    </div>
...
function RefAssetCard({ asset }: { asset: AnnotatedAssetRecord & { distance: number } }) {
  return (
    <div className="overflow-hidden rounded-xl border border-stone-200 bg-white shadow-sm">
      ...
      <div className="p-3">
        <div className="flex items-center justify-between gap-3 text-xs text-stone-700"><span className="truncate font-medium text-stone-800">{asset.title}</span><span className="shrink-0 text-stone-500">dist {asset.distance.toFixed(2)}</span></div>
        <div className="mt-1 text-[11px] text-stone-500">source {asset.annotation?.annotationSource ?? (asset.tags?.includes("extended") ? "extended-registry" : "unknown")}</div>
      </div>
    </div>
  );
}

function VariantCardView({ variant, feedback, matchedAsset, onFeedback, onFinalize }: { variant: VariantCard; feedback?: "liked" | "disliked"; matchedAsset?: AnnotatedAssetRecord & { distance: number }; onFeedback: (variantId: string, value: "liked" | "disliked") => void; onFinalize: (variant: VariantCard) => void; }) {
  return (
    <div className="rounded-[28px] border border-stone-200 bg-white p-4 shadow-sm">
      <MatchedFuliPreview title="Matched ref" state={variant.state} matchedAsset={matchedAsset} badge="variant ref" />
      <div className="mt-4 flex items-start justify-between gap-3">
        ...
        <button onClick={() => onFinalize(variant)} className="inline-flex items-center gap-1 rounded-full border border-amber-300 bg-amber-50 px-3 py-1 text-xs font-medium text-amber-700"><Trophy className="h-3.5 w-3.5" /> Final</button>
      </div>
...
export function SimulatorPage() {
  ...
  const handleContinue = () => {
    ...
    const chosenRefIds = [baseNearestRefs[0]?.imageId, preferenceRef?.imageId, ...Object.values(variantNearestRefsMap).flat().map((asset) => asset.imageId)].filter(Boolean) as string[];

    setSeenRefIds((prev) => [...prev, ...chosenRefIds]);
    ...
    <div className="min-h-screen bg-[linear-gradient(180deg,#f7f3ee_0%,#f1ebe2_100%)] text-stone-900">
      <div className="mx-auto max-w-7xl px-6 py-8 lg:px-10">
        ...
        <div className="grid gap-6 lg:grid-cols-[1.1fr_1.6fr]">
          <div className="space-y-6">
            <div className="rounded-[28px] border border-stone-200 bg-white p-5 shadow-sm">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <MatchedFuliPreview title="Base ref" state={baseState} matchedAsset={baseNearestRefs[0]} badge="base ref" />
            ...
            <div className="rounded-[28px] border border-stone-200 bg-white p-5 shadow-sm">
              ...
              <div className="mb-4 rounded-2xl border border-stone-200 bg-stone-50 p-3 text-xs text-stone-600">当前 source：<span className="font-semibold text-stone-800">{assetSourceMode}</span> · seen refs {seenRefIds.length}</div>
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                {baseNearestRefs.map((asset) => (
                  <div key={asset.imageId} className="space-y-2">
                    <RefAssetCard asset={asset} />
                    <div className="rounded-xl border border-stone-200 bg-stone-50 px-3 py-2 text-xs text-stone-500">confidence {asset.annotation?.confidence ?? "n/a"}<div className="mt-2 flex flex-wrap gap-2">{(asset.roleMap?.roles ?? []).map((role: string) => <span key={role} className="rounded-full bg-white px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-stone-600">{role}</span>)}</div><div className="mt-2">strong: {(asset.roleMap?.strongSlots ?? []).join(", ") || "-"}</div></div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="rounded-[28px] border border-stone-200 bg-white p-5 shadow-sm">
              ...
              {anchors.length === 0 ? <p className="text-sm text-stone-500">还没有保留下来的喜欢卡牌。先在当前 round 里标记喜欢项。</p> : <div className="space-y-3">{anchors.map((anchor) => <div key={`${anchor.id}-${anchor.round}`} className="rounded-2xl border border-emerald-200 bg-emerald-50/70 p-3"><div className="flex items-center justify-between text-sm font-medium text-stone-800"><span>{anchor.label}</span><span className="text-xs text-stone-500">Round {anchor.round}</span></div><div className="mt-1 text-xs text-stone-500">{anchor.summary}</div></div>)}</div>}
            </div>
          ...
          <div>
            <div className="mb-4 flex items-center justify-between"><div><div className="text-xs font-semibold uppercase tracking-[0.2em] text-stone-500">Current variants</div><div className="mt-1 text-sm text-stone-600">当前先 mock 4 张变体卡；大多数轮次围绕一阶参数，少数轮次由二阶参数调制一阶趋势。</div></div></div>
            <div className="grid gap-5 xl:grid-cols-2">
              {variants.map((variant) => <VariantCardView key={variant.id} variant={variant} feedback={feedbackMap[variant.id]} matchedAsset={variantNearestRefsMap[variant.id]?.[0]} onFeedback={handleFeedback} onFinalize={(nextFinal) => setFinalChoice(nextFinal)} />)}
            </div>
          </div>
        </div>
      </div>
