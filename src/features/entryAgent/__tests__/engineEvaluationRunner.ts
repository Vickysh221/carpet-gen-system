  ...
  return {
    ...
    planner: {
      frontstageResponsePlan: {
        ...
        compositionProposals: (analysis.frontstageResponsePlan?.compositionProposals ?? []).map((item) => ({
          id: item.id,
          title: item.title,
          summary: item.summary,
          dominantHandles: item.dominantHandles,
          suppressedHandles: item.suppressedHandles ?? [],
          blendNotes: item.blendNotes ?? [],
        })),
